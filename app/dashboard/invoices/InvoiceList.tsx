...
import { createInvoiceAction, InvoiceInputSchema } from "./actions";

export default function InvoiceList({ invoices, clients, products }: { invoices: any[]; clients: any[]; products: any[] }) {
  return (
    <>
      <InvoiceEntries invoices={invoices} />
      <AddInvoiceForm clients={clients} products={products} />
    </>
  );
}

function InvoiceEntries({ invoices }: { invoices: any[] }) {
  if (!invoices.length) {
    return (
